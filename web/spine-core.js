...
var spine;
(function (spine) {
    var SpineScaler = (function () {
        function SpineScaler() {
            this.scale = 1.0;
        }
        SpineScaler.prototype.readJosnScale = function (json_data, atlas_data) {
            var atlas = new spine.TextureAtlas(atlas_data, function (xx) { console.log(xx); });
            var json = new spine.SkeletonJson(null);
            var first = true;
            json.readSkeletonData(json_data);
            for (var key in atlas.text_config) {
                if (json.text_config[key]) {
                    var scale_w = atlas.text_config[key].width / json.text_config[key].width;
                    var scale_h = atlas.text_config[key].height / json.text_config[key].height;
                    if (first == true) {
                        first = false;
                        this.scale = scale_w;
                    }
        ...
        SpineScaler.prototype.readSkelScale = function (skel_data, atlas_data) {
            var atlas = new spine.TextureAtlas(atlas_data, function (xx) { console.log(xx); });
            var skel = new spine.SkeletonBinary(null);
            skel.readSkeletonData(skel_data);
            var first = true;
            for (var key in atlas.text_config) {
                if (skel.text_config[key]) {
                    var scale_w = atlas.text_config[key].width / skel.text_config[key].width;
                    var scale_h = atlas.text_config[key].height / skel.text_config[key].height;
                    if (first == true) {
                        first = false;
                        this.scale = scale_w;
                    }
...
(function (spine) {
    var SkeletonBinary = (function () {
        ...
        SkeletonBinary.prototype.readSkeletonData = function (binary) {
            ...
            this.version = skeletonData.version;
        ...
        SkeletonBinary.prototype.readAttachment = function (input, skeletonData, skin, slotIndex, attachmentName, nonessential) {
            ...
            switch (type) {
                ...
                case spine.AttachmentType.Mesh: {
                    ...
                    this.text_config[mesh.path] = new spine.SkelSlice(mesh.path, mesh.width, mesh.height);
                    return mesh;
                }
        ...
        SkeletonJson.prototype.readAttachment = function (map, skin, slotIndex, name, skeletonData) {
            ...
            switch (type) {
                case "region": {
                    ...
                    this.text_config[region.name] = new spine.SkelSlice(region.name, region.width, region.height);
                    return region;
                }
                case "mesh":
                case "linkedmesh": {
...
var spine;
(function (spine) {
    var SkelSlice = (function () {
        function SkelSlice(name, w, h) {
            this.name = name;
            this.width = w;
            this.height = h;
        }
